...
from visited_companies import visited
...
def extractData(sub):
    ...
    # no search results
    if row_count + 2 == 0:
        ws2.row(row_noResults).write(1, sub) # add to list of subsidiaries that do not have any search results in sheet 2
        row_noResults += 1
        wb.save('/Users/alethea/Downloads/Spreadsheet_test.xls') # TO AMEND
...
if __name__ == '__main__':
    chromedriver_path = '/Users/alethea/Documents/chromedriver-2' # TO AMEND
    driver = webdriver.Chrome(executable_path=chromedriver_path)
    # open the data
    # data = open_workbook('data.xlsx') # TO AMEND
    # to continue from previous work
    # visited_companies = open_workbook('Results_So_Far.xlsx') # TO AMEND
...
#     subSearch("MetroPCS AWS, LLC")
    ...
    # createDict(data) returns a set of all the subsidiary companies (so this removes duplicates)
    # companies_left = createDict(data) - visited(visited_companies)

    # companies = ['SprintCom, Inc.', 'NSIGHTTEL WIRELESS, LLC', 'DAREDEVIL COMMUNICATIONS LLC', 'Buffalo-Lake Erie Wireless Systems Co., L.L.C.', 'East Kentucky Network, LLC', 'N.E. Colorado Wireless Technologies, Inc.', 'Eastern Kansas Wireless Consortium', 'Toba Inlet PCS, LLC', 'St. Cloud Wireless Holdings, LLC', 'Pioneer Telephone Cooperative, Inc.', 'Highland Cellular Holdings, Inc.', 'AST Telecom, LLC', 'American Samoa Telecommunications Authority', 'GreenFly LLC']

    # companies = ['East Kentucky Network, LLC', 'N.E. Colorado Wireless Technologies, Inc.', 'Eastern Kansas Wireless Consortium', 'Toba Inlet PCS, LLC', 'St. Cloud Wireless Holdings, LLC', 'Pioneer Telephone Cooperative, Inc.', 'Highland Cellular Holdings, Inc.', 'AST Telecom, LLC', 'American Samoa Telecommunications Authority', 'GreenFly LLC']

    companies = ['Highland Cellular Holdings, Inc.', 'AST Telecom, LLC', 'American Samoa Telecommunications Authority', 'GreenFly LLC']

    # actual implementation:
    for subsidiary in companies:
        subSearch(subsidiary)
        wb.save('/Users/alethea/Downloads/Natalie_Jiaqi.xls') # TO AMEND

    # write the list of subsidiaries that are returning results in sheet 2 col 1
    # subsWithResults = subsWithResults.union(visited(visited_companies))
